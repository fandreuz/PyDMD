...
class DMDOperator:
    ...
    def compute_operator(self, X, Y):
        ...
        VV = linalg_module.dot(V, V.T.conj())
        identity = linalg_module.to(VV, np.identity(len(VV)))
    ...
    def phase_space_prediction(self):
        if not hasattr(self, "_phase_space_prediction") or self._phase_space_prediction is None:
            raise ValueError("You need to call fit before")
        else:
